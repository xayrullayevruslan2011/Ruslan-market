...
// API: Save to Code (writes to data.json and src/data/db.ts)
app.post("/api/save-to-code", async (req, res) => {
  try {
    const data = await loadData();
    // Save to data.json
    fs.writeFileSync(DATA_FILE, JSON.stringify(data, null, 2));
    
    // Save to src/data/db.ts for permanent persistence in source code
    const dbFilePath = path.join(process.cwd(), "src", "data", "db.ts");
    const dbContent = `import { Product, User, Order, Banner } from '../../types';
...
`;
    ...
    console.log(`[Server] Data saved to ${DATA_FILE} and ${dbFilePath}`);
    res.json({ success: true, message: "Ma'lumotlar kodga doimiy saqlandi. Sayt qayta yuklanganda ham o'chib ketmaydi." });
  } catch (e: any) {
    console.error('[Server] Save to code error:', e);
